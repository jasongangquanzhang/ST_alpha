...
class ST_alpha_env:
    ...
    def step(
        ...
        Returns:
            t_p (torch.Tensor): Next time step, shape (Nsims,)
        ...
        if done.any():
            ...
            reward = reward - mask * (self.varphi * (q_p**2))-mask*(0.5 * self.Delta * torch.sign(q_p)) * q_p
            # Clear inventory at terminal
            q_p = q_p * (1 - mask)
        reward = self.reward_scale * reward
        return t_p, S_p, X_p, alpha_p, q_p, reward, isMO, buySellMO, done

class ST_alpha_env_Telescoping:
    ...
    def Randomize_Start(
        self, t, mini_batch_size=10
    ):  #  in each time of the training, all things should be randomized
