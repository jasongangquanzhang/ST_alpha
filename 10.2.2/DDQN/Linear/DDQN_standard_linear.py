...
from env_standard_linear import StandardEnv
...
class ReplayBuffer:
    ...
    def push(self, state, action, reward, next_state):
        ...
        for i in range(batch_size):
            s, a, r, ns = state[i], action[i], reward[i], next_state[i]
    ...
    def sample(self, batch_size):
        ...
        random_indices = torch.randperm(len(self.buffer))[:batch_size]
        states, actions, rewards, next_states = zip(*(self.buffer[i] for i in random_indices))
...
class DDQNAgent:
    def __init__(self, env: StandardEnv, state_size, action_size, seed, lr=1e-3,
        ...
        Args:
            ...
            sample_size (int): Number of experiences to sample from the replay buffer for learning.
    ...
    def train(self, n_episode, n_plot, eps_start, eps_end, eps_decay, batch_size, num_updates_per_episode):
        """
        Args:
            ...
            num_updates_per_episode (int): Number of updates to Q-network per episode.
        """
        eps = eps_start
        total_rewards = []
        average_rewards = []     # Average reward per episode
        ...
        for i in tqdm(range(n_episode)):
            ...
            t, S, q, X = self.env.Randomize_Start(batch_size)

            episode_reward = 0

            for j in range(self.env.Ndt):
                # Choose an action to take in the current state using epsilon-greedy policy
                ...
                # Store in the replay buffer and step the agent
            ...
            average_rewards.append(episode_reward / batch_size)

            if i != 0 and i % n_plot == 0:
                for param_group in self.optimizer.param_groups:
                    print(f"Learning Rate: {param_group['lr']}")
                print(f"Iteration {i}, Epsilon: {eps}, Total Reward: {reward.mean().item()}")
                self.plot_training_batch_full(t, S, X, q, action, reward, i)
                self.loss_plots()
        ...
        fig, axs = plt.subplots(1, 2, figsize=(8, 6), sharex=False)
    ...
    def plot_training_batch_full(self, t, S, X, q, action, reward, iteration):
        """
        ...
        axes[0, 2].hist(a, bins=np.arange(self.action_size + 1) - 0.5, color='orange', edgecolor='black', rwidth=0.8)
        axes[0, 2].set_xticks(range(self.action_size))
        axes[0, 2].set_xticklabels(['nothing', 'buy', 'sell', 'buy/sell'], rotation=45)
        axes[0, 2].set_title("Action Distribution")

        # Scatter plots
        axes[1, 0].scatter(q, a, alpha=0.5, c=r, cmap='coolwarm', s=10)
        axes[1, 0].set_xlabel("Inventory")
        axes[1, 0].set_ylabel("Action")
        axes[1, 0].set_title("Action vs Inventory")

        axes[1, 1].scatter(S, r, alpha=0.5, s=10)
        axes[1, 1].set_xlabel("Midprice")
        axes[1, 1].set_ylabel("Reward")
        axes[1, 1].set_title("Reward vs Midprice")

        axes[1, 2].scatter(a, r, alpha=0.5, s=10)
        axes[1, 2].set_xlabel("Action")
        axes[1, 2].set_ylabel("Reward")
        axes[1, 2].set_title("Reward vs Action")

        # More diagnostics
        axes[2, 0].scatter(t, q, alpha=0.5, s=10)
        axes[2, 0].set_xlabel("Time")
        axes[2, 0].set_ylabel("Inventory")
        axes[2, 0].set_title("Inventory vs Time")

        axes[2, 1].scatter(t, r, alpha=0.5, s=10)
        axes[2, 1].set_xlabel("Time")
        axes[2, 1].set_ylabel("Reward")
        axes[2, 1].set_title("Reward vs Time")

        axes[2, 2].scatter(q, r, alpha=0.5, s=10)
        axes[2, 2].set_xlabel("Inventory")
        axes[2, 2].set_ylabel("Reward")
        axes[2, 2].set_title("Reward vs Inventory")

        plt.suptitle(f"Batch Diagnostics - Iteration {iteration}", fontsize=16)
        plt.tight_layout()
